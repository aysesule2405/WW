...
export default function SpiritSaplingGame({ onExit }: Props) {
  ...
  // detect when a stage asset is one of the sacred fruit trees (final-stage assets)
  const sacredFruitPaths = guardians.map((g) => g.fruitTree);
  const previousStageSrc = previousStageIndex !== null ? growthStages[previousStageIndex] : null;
  const isCurrentFruitTree = typeof currentStageSrc === 'string' && sacredFruitPaths.includes(currentStageSrc);
  const isPreviousFruitTree = typeof previousStageSrc === 'string' && sacredFruitPaths.includes(previousStageSrc as string);
  ...
  // auto-save progress for logged-in users when harvest completes
  useEffect(() => {
    if (!hasCollectedFruit || !harvestedGuardian) return
    ...
    ;(async () => {
      try {
        ...
        setSaveMessage('Saving your harvest...')
        const res = await api.saveProgress({ gameSlug: 'spirit-sapling', levelReached: 100, xp: 0, completionPercent: 100 })
        if (cleared) return
        if (res?.error) {
          setSaveStatus('error')
    ...
    return () => { cleared = true }
  }, [hasCollectedFruit, harvestedGuardian])
              ...
              <div style={{ marginTop: 10 }}>
                <div
                  style={{
                    ...
                    padding: '8px 12px',
                    borderRadius: 12,
                    background: saveStatus === 'success' ? 'rgba(144, 238, 144, 0.12)' : 'rgba(255, 200, 200, 0.08)',
                    color: saveStatus === 'success' ? '#B6E7A4' : '#FFB6B6',
          ...
          <div style={styles.saplingFrame}>
            {previousStageIndex !== null ? (
              <img
                ...
                style={isCurrentStageLoaded
                  ? { ...styles.saplingImagePrevious, ...(isPreviousFruitTree ? styles.saplingFruitFill : {}) }
                  : { ...styles.saplingImagePreviousHold, ...(isPreviousFruitTree ? styles.saplingFruitFill : {}) }
                }
              />
            ) : null}
            <img
              ...
              style={isCurrentStageLoaded ? { ...styles.saplingImageCurrent, ...(isCurrentFruitTree ? styles.saplingFruitFill : {}) } : styles.saplingImageCurrentHidden}
              onLoad={() => {
...
const styles: Record<string, React.CSSProperties> = {
  ...
  celebrationSapling: {
    ...
    borderRadius: 20,
    boxShadow: '0 30px 70px rgba(6,10,8,0.6)',
