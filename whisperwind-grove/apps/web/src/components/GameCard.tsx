...
export default function GameCard({ game, onPlay, revealDelayMs = 0 }: Props) {
  ...
  // responsive: detect narrow viewports to reduce image height on mobile
  ...
  // listen for changes (modern browsers support addEventListener on MediaQueryList)
  ...
  const styles: { [k: string]: React.CSSProperties } = {
    ...
    imageWrap: {
      width: '100%',
      // height is set dynamically per-component to support responsive sizes
      borderRadius: 10,
  ...
  return (
    ...
    >
      {/* responsive image area: larger on desktop, smaller on mobile */}
      {(() => {
        // compute responsive image parameters
        const imageHeight = game.gameBg ? (isMobile ? 140 : 230) : (isMobile ? 110 : 160);
        const imagePadding = game.gameBg ? 0 : 8;
        const imageObjectFit = game.gameBg ? 'cover' : 'contain';

        return (
          <div style={{ ...styles.imageWrap, height: imageHeight, padding: imagePadding }}>
            {cardImage ? (
              <img src={cardImage} alt={game.title} style={{ ...styles.image, ...( { objectFit: imageObjectFit } as React.CSSProperties ) }} />
            ) : (
