...
import { serializeUserProfile } from './userProfile'
...
export const updateProfile = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ error: 'User not found' })
    return res.status(200).json({ updated: true, ...serializeUserProfile(user) })
  } catch (err: any) {
...
export const uploadAvatar = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const user = await userRepo.updateProfile(userId, { avatarUrl: avatarBase64, avatarPreference: 'photo' })
    if (!user) return res.status(404).json({ error: 'User not found' })
    return res.status(200).json({ updated: true, ...serializeUserProfile(user) })
  } catch (err: any) {
    return res.status(500).json({ error: err.message })
